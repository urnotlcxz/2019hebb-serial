...
float inputs[INPUT_NUM];
float inputs_w[INPUT_NUM];
float inputs_div[DIMENSION][INPUT_NUM];

struct NN
{
	float w[SINGLE_WEIGHT];
	float input_w[INPUT_NUM];
...
void initial_chromo(struct CHROMO chromo[NUM_CHROMO])
{
	...
	{
		for (int kkk = 0; kkk < INPUT_NUM; kkk++)
		{
			chromo[i].nn_initial.input_w[kkk] = (float)rand() / RAND_MAX * 2 - 1;
		}

		for (int kkk = 0; kkk < SINGLE_WEIGHT; kkk++)
		{
			chromo[i].nn_initial.w[kkk] = (float)rand() / RAND_MAX * 2 - 1;
		}

		chromo[i].A = (float)rand() / RAND_MAX * 2 - 1;
		chromo[i].B = (float)rand() / RAND_MAX * 2 - 1;
		chromo[i].C = (float)rand() / RAND_MAX * 2 - 1;
		chromo[i].D = (float)rand() / RAND_MAX * 2 - 1;
...
void  nn_forward_and_update_paramenter(struct CHROMO ch[], int num, float f, int num_fun3, int num_epoch, float delta_p[])  ////
{
	...
	inputs[1] = ch[num].position[num_fun3][i] / MAX;
	inputs[2] = ch[num].position[num_fun3][i];
	...
	for (i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
	{
		...
		if (i < INPUT_NUM)
		{
			ch[num].nn[num_fun3].new_neural[i] = sigmoid(ch[num].nn[num_fun3].new_neural[i] + ch[num].nn_initial.input_w[i] * inputs[i]);
		}
		else
		{
...
void  nn_forward_and_update_paramenter(struct CHROMO ch[], int num, float f, int num_fun3, int num_epoch, float delta_p[])  ////
{
	...
	for (k = 0; k < DIMENSION; k++)						//前向传递
	{
		...
		for (i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
		{
			...
			if (i<INPUT_NUM)
			{
				ch[num].nn[num_fun3].new_neural[i] = sigmoid(ch[num].nn[num_fun3].new_neural[i] + ch[num].nn_initial.input_w[i] * inputs_div[k][i]);
			}
			else
			{
				ch[num].nn[num_fun3].new_neural[i] = sigmoid(ch[num].nn[num_fun3].new_neural[i]);
...
int main()
{
	...
	for (num_epoch = 0; num_epoch < THREAD_EPOCH; num_epoch++)
	{
		...
		for (int i = 0; i < DIMENSION; i++)
		{
			//printf("neural out of function:   %f\n",ch[idx / (NUM_BENCHMARK*PER_BENCHMARK_TIMES)].nn[idx % (NUM_BENCHMARK*PER_BENCHMARK_TIMES)][i].old_neural[NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 1]);
			//printf("position0:  %f\n", ch[idx / (NUM_BENCHMARK*PER_BENCHMARK_TIMES)].position[idx % (NUM_BENCHMARK*PER_BENCHMARK_TIMES)][i]);
			ch[0].position[0][i] += (delta_p[i] * (DELTA_X_MAX - DELTA_X_MIN) - DELTA_X_MAX)* MAX;
			//	printf("delta_x:%f\n", (ch[0].nn[0][i].old_neural[NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 1] * (DELTA_X_MAX - DELTA_X_MIN) - DELTA_X_MAX)* MAX);
			//printf("delta_X_p:%f\n", (ch[0].nn[0][i].old_neural[NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 1]));
			/*value_position = ch[0].position[0][i];
