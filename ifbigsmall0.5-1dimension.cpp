...
#define DIMENSION 1
#define NUM_NEURAL_INPUT 2   //一维一维输进去
...
int main()
{
	...
	for (num_epoch = 0; num_epoch < THREAD_EPOCH; num_epoch++)
	{
		for (int k = 0; k < DIMENSION; k++)
		{
			...
			fprintf(f_position, "%f\t", ch[0].position[0][k]);
		}
		fprintf(f_position, "\n");
		...
		for (int i = 0; i < DIMENSION; i++)
		{
			...
			if (delta_p[i] <= 0.5)
			{
				ch[0].position[0][i] += (-1 * (float)rand() / (RAND_MAX + 1.0)* (DELTA_X_MAX - DELTA_X_MIN) - DELTA_X_MAX)* MAX;
			}
			else
			{
				ch[0].position[0][i] += ((float)rand() / (RAND_MAX + 1.0)* (DELTA_X_MAX - DELTA_X_MIN) - DELTA_X_MAX)* MAX;
			}
		}
