...
// 1dimension.cpp : Defines the entry point for the console application.
//输入f和一维，输出两个相减
...
#define NUM_NEURAL_HIDDEN 3
#define NUM_NEURAL_OUTPUT 2
#define EPOCH 1
...
float inputs[DIMENSION][NUM_NEURAL_INPUT];
...
void initial_chromo(struct CHROMO chromo[NUM_CHROMO])
{
	...
	for (int i = 0; i < NUM_CHROMO; i++)
	{
		...
		for (int num_fun = 0; num_fun < NUM_BENCHMARK*PER_BENCHMARK_TIMES; num_fun++)
		{
			...
			{
				chromo[i].initial_position[num_fun][k] = (float)rand() / RAND_MAX * (MAX - MIN) - MAX;
				chromo[i].position[num_fun][k] = chromo[i].initial_position[num_fun][k];
			}
...
void  nn_forward_and_update_paramenter(struct CHROMO ch[], int num, float f, int num_fun3, int num_epoch, float delta_p[])  ////
{
	...
	for (int k = 0; k < DIMENSION; k++)						//前向传递
	{
		...
		for (int i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
		{
			...
			{
				ch[num].nn[num_fun3].new_neural[i] = sigmoid(ch[num].nn[num_fun3].new_neural[i] + inputs[k][i] * ch[num].nn[num_fun3].input_w[i]);
			}
			else
			{
...
int main()
{
	...
	for (num_epoch = 0; num_epoch < THREAD_EPOCH; num_epoch++)
	{
		...
		for (int k = 0; k < DIMENSION; k++)
		{
			...
			ch[0].position[0][k] += (((ch[0].nn[0].old_neural[NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 1] * DELTA_X_MAX + ch[0].nn[0].old_neural[NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 2] * DELTA_X_MIN) ))* MAX;
		}
