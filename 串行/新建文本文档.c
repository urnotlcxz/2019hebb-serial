...
float inputs[DIMENSION][INPUT_NUM];
...
void initial_chromo(struct CHROMO chromo[NUM_CHROMO])
{
	...
	for (i = 0; i < NUM_CHROMO; i++)
	{
		...
		{
			chromo[i].short_dis[num_fun] = 100.0;
...
void  nn_forward_and_update_paramenter(struct CHROMO ch[], int num, float f, int num_fun3, int num_epoch)  ////
{
	...
	for (i = 0; i < DIMENSION; i++)
	{
		...
		for (int j = 0; j < 5; j++)
		{
			fprintf(f_old_neural, "%f\t", ch[num].nn[num_fun3][i].old_neural[j]);
		}
	...
	for (k = 0; k < DIMENSION; k++)						//前向传递
	{
		...
		for (i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
		{
			...
			if (i<INPUT_NUM)
			{
				ch[num].nn[num_fun3][k].new_neural[i] = sigmoid(ch[num].nn[num_fun3][k].new_neural[i] + ch[num].nn_initial.input_w[i] * inputs[k][i]);
			}
			else
			{
...
int main()
{
	...
	for (num_epoch = 0; num_epoch < THREAD_EPOCH; num_epoch++)
	{
		...
		printf("in_fitness[%d]:  %f\n", num_epoch, in_fitness);
	}
	printf("%f\n", sum_delta_w);
}