// addDimension.cpp : Defines the entry point for the console application.
//
...
struct NN
{
	...
	float output_w[DIMENSION];
	float output_b[DIMENSION];
...
void initial_chromo(struct CHROMO chromo[NUM_CHROMO])
{
	...
	for (int i = 0; i < NUM_CHROMO; i++)
	{
		...
		for (int k = 0; k < NUM_NEURAL_OUTPUT; k++)
		{
			chromo[i].nn_initial.output_w[k] = (float)rand() / RAND_MAX *(INI_W_MAX - INI_W_MIN) - INI_W_MAX;
			chromo[i].nn_initial.output_b[k] = (float)rand() / RAND_MAX *(INI_W_MAX - INI_W_MIN) - INI_W_MAX;
		}
		...
		for (int num_fun = 0; num_fun < NUM_BENCHMARK*PER_BENCHMARK_TIMES; num_fun++)
		{
			...
			for (int t = 0; t < NUM_NEURAL_OUTPUT; t++)
			{
				chromo[i].nn[num_fun].output_w[t] = chromo[i].nn_initial.output_w[t];
				chromo[i].nn[num_fun].output_b[t] = chromo[i].nn_initial.output_b[t];
			}
...
void  nn_forward_and_update_paramenter_bias_on_out(struct CHROMO ch[], int num, float f, int num_fun3, int num_epoch, float delta_p[])  ////
{
	FILE *f_new_neural, *f_delta_w, *f_old_w, *f_new_w;
	FILE *f_old_neural;
	/*
	...
	*/
	...
	{
		if (j % (NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 1) == 0)
		{
			fprintf(f_old_w, "\n\n");
		}
	...
	float neural[DIMENSION][NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT];
	...
	*/
	...
	for (int k = 0; k < DIMENSION; k++)
	{
		...
		for (int i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
		{
			...
			neural[k][i] = ch[num].nn[num_fun3].new_neural[i];
		}
	...
	for (int i = 0; i < DIMENSION; i++)
	{
		for (int j = 0; j < DIMENSION; j++)
		{
			if (i == j)
			{
				delta_p[i] += neural[j][NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 1];
			}
			else
			{
				delta_p[i] += neural[j][NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT - 1] * ch[num].nn[num_fun3].output_w[j] + ch[num].nn[num_fun3].output_b[j];
			}
		}
	}


	float delta_neural[NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT];
	//更新神经元
	for (int i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
	{
		for (int j = 0; j < DIMENSION; j++)
		{
			delta_neural[i] += neural[j][i];
		}
		delta_neural[i] = delta_neural[i] / DIMENSION;
	}
	for (int i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
	{
		ch[num].nn[num_fun3].old_neural[i] = delta_neural[i];
	}
	//fclose(f_old_neural);
	fclose(f_new_neural);
	fclose(f_new_w);
...
int main()
{
	...
	for (num_epoch = 0; num_epoch < THREAD_EPOCH; num_epoch++)
	{
		...
		{
			nn_forward_and_update_paramenter_bias_on_out(ch, 0, ff, 0, (num_epoch + 1), delta_p);
		}
		for (int i = 0; i < DIMENSION; i++)
		{
		...
		for (int j = 0; j < (NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT); j++)
		{
			fprintf(f_old_neural, "%f\t", ch[0].nn[0].old_neural[j]);
		}
		fprintf(f_old_neural, "\n");
		for (int i = 0; i < NUM_NEURAL_INPUT; i++)
		{
