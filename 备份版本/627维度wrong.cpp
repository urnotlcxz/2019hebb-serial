...
void initial_chromo(struct CHROMO chromo[NUM_CHROMO])
{
	...
	for (i = 0; i < NUM_CHROMO; i++)
	{
		...
		{
			chromo[i].nn_initial.w[t]=(float)rand() / RAND_MAX * 2 - 1;
		}
...
void  nn_forward_and_update_paramenter(struct CHROMO ch[], int num, float f, int num_fun3, int num_epoch,float delta_p[])  ////
{
	...
	for (k = 0; k < DIMENSION; k++)						//前向传递
	{
		...
		for (i = 0; i < NUM_NEURAL_HIDDEN + NUM_NEURAL_INPUT + NUM_NEURAL_OUTPUT; i++)
		{
			...
			if(i<INPUT_NUM)
			{
				ch[num].nn[num_fun3].new_neural[i] = sigmoid(ch[num].nn[num_fun3].new_neural[i] + ch[num].nn_initial.input_w[i]*inputs[k][i]);
			}
			else
			{
		...
		delta_p[k]=ch[num].nn[num_fun3].old_neural[k];
	}
